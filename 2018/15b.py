...
class Board:
	...
	def damage(self, position, amount):
		unit = self[position]
		assert(type(unit) == Unit)
		unit.health -= amount
		if unit.health <= 0:
...
def play(board, elf_strength):
	...
	while not board.done():
		...
		for my_position, my_field in units:
			...
			my_faction = my_field.faction
			enemy_faction = my_faction.enemy()
			...
			if enemy_position is not None:
				if my_faction == Faction.ELVES:
					strength = elf_strength
				else:
					strength = GOBLIN_STRENGTH
				death = board.damage(enemy_position, strength)
				if death and enemy_faction == Faction.ELVES:
					return None
...
while True:
	if VERBOSE2:
		print('checking strength', elf_strength, '...')
	board = Board(board_string)
	num_completed_rounds = play(board, elf_strength)
	if num_completed_rounds is not None:
		break
	elif VERBOSE2:
		print('an elf died')
	elf_strength += 1
