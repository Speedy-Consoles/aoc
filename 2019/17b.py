...
def get_moves(board):
	...
	for y in range(height):
		for x in range(width):
			field = board[y][x]
			if field not in DIR_CHARS:
				continue
			my_dir = DIR_CHARS[field]
			my_x = x
			my_y = y
			break
		if my_x is not None:
	...
	while True:
		...
		if next_dir is None:
			return moves
		if (my_dir + 1) % 4 == next_dir:
			moves.append('R')
		elif (my_dir + 3) % 4 == next_dir:
			moves.append('L')
		my_dir = next_dir

		counter = 0
		while y >= 0 and y < height and x >= 0 and x < width and board[y][x] == '#':
			counter += 1
			my_x = x
			my_y = y
			x += DIRS[my_dir][0]
...
computer = IntcodeComputer(mem)
output_iterator = computer.run()
out = ''
last_char = None

for c in output_iterator:
	if c == 10 and last_char == 10:
		break
	out += chr(c)
	last_char = c
#print(out)

board = out.strip().split('\n')
moves = get_moves(board)
#print(moves)

sequence, a, b, c = compress(moves)
sequence_string = ','.join(sequence)
a_string = ','.join(str(x) for x in a)
b_string = ','.join(str(x) for x in b)
c_string = ','.join(str(x) for x in c)
input_string = '\n'.join([sequence_string, a_string, b_string, c_string, 'n', '\n'])
#print(input_string)

computer.set_input(ord(x) for x in input_string)

last_char = None
for c in computer.run():
	last_char = c
print(last_char)
