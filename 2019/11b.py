...
y = 0
move_dir = 0
computer = IntcodeComputer(initial_mem, input_iter)
for paint_color, rot_dir in zip(*([computer.run()] * 2)):
	if paint_color not in (0, 1):
		raise Exception('Invalid paint color')

	grid[(x, y)] = paint_color
	if rot_dir == 0:
	...
	input_iter.put(grid[(x, y)])

xmin = min(f[0] for f in grid)
ymin = min(f[1] for f in grid)
xmax = max(f[0] for f in grid)
ymax = max(f[1] for f in grid)

palette = {0: ' ', 1: '█'}
for y in range(ymax, ymin - 1, -1):
	print(''.join(palette[grid[(x, y)]] for x in range(xmin, xmax + 1)))
