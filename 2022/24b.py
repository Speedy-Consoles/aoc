...
def search(start, target, valley):
	DIRS = [
		Vec(1, 0),
		Vec(0, 1),
		Vec(-1, 0),
	...
	while not fringe.empty():
		...
		for off in DIRS:
			...
			next_valley = get_valley(valleys, next_dist)
			if next_pos == target:
				return next_dist, next_valley
			elif (next_pos, next_dist) not in visited and (next_pos == start or next_valley.free(next_pos)):
...
start = Vec(0, -1)
valley = Valley.from_lines(lines)
target = Vec(valley.width - 1, valley.height)
there, valley = search(start, target, valley)
back, valley = search(target, start, valley)
there_again, valley = search(start, target, valley)

print(there + back + there_again)
