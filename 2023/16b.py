...
def shoot_beam(grid, start_position, start_direction_index):
	beams = [(start_position, start_direction_index)]
	energized_tiles = {}
...
def shoot_beams(grid):
	width = len(grid[0])
	height = len(grid)
	for y in range(height):
		yield shoot_beam(grid, Vec(0, y), 0)
		yield shoot_beam(grid, Vec(width - 1, y), 2)
	for x in range(width):
		yield shoot_beam(grid, Vec(x, 0), 1)
		yield shoot_beam(grid, Vec(x, height - 1), 3)

print(max(shoot_beams(grid)))
